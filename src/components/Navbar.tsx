...
//                 DPM Properties
//               </h1>
//               <p className="text-base md:text-2xl font-bold text-Black-500 tracking-wide drop-shadow-md">
//                 Real Estate
//               </p>
//             </div>
...
//               key={item.name}
//               href={item.to}
//               className="text-gray-800 hover:text-yellow-600 transition-colors duration-200 font-medium flex items-center group"
//             >
...
//         {/* <a
//           href="#contact"
//           className="hidden md:flex golden-gradient text-white font-medium py-2 px-6 rounded-full hover:shadow-lg transition-all duration-300 items-center hover:scale-105"
//         >
...
//         <a
//           href="#contact"
//           className="hidden md:flex border-2 border-yellow-600 text-yellow-600 hover:bg-yellow-600 hover:text-white font-medium py-2 px-6 rounded-full transition-all duration-300 items-center hover:scale-105"
//         >
//           <Phone size={16} className="mr-2" />
//           Get in touch
//         </a>
...
//           <a
//             href="#contact"
//             className="yellow-gradient text-white font-medium py-3 px-8 rounded-full mt-4 text-center w-full flex items-center justify-center"
//             onClick={() => setIsOpen(false)}
//           >
      ...
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <a href="#home" className="flex items-center z-50 relative">
          <div className={cn(
            ...
            />
            <div className="flex flex-col items-start leading-none">
              <h1 className="font-extrabold text-2xl md:text-4xl text-yellow-600 drop-shadow-sm whitespace-nowrap">
                DPM Properties
              </h1>
              <p className="text-base md:text-2xl font-bold text-yellow-500 tracking-wide drop-shadow-md">
                Real Estate
              </p>
            </div>
          </div>
        ...
        <div className="hidden md:flex space-x-8">
          {navItems.map((item) => (
            <a
              key={item.name}
              href={item.to}
              className="text-yellow-600 hover:text-yellow-500 transition-colors duration-200 font-medium flex items-center group"
            >
              <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 transform -translate-x-2 group-hover:translate-x-0">
                {item.icon}
              </span>
        ...
        <button
          id="menu-toggle"
          onClick={toggleMenu}
          className="md:hidden text-yellow-600 hover:text-yellow-500 transition-colors duration-200 z-50 relative"
          aria-label="Toggle menu"
          aria-expanded={isOpen}
        >
          <div className="relative h-6 w-6 flex items-center justify-center">
        ...
        <div className="flex flex-col items-center space-y-6 p-8">
          {navItems.map((item) => (
            <a
              key={item.name}
              href={item.to}
              className="text-xl text-yellow-600 hover:text-yellow-500 transition-colors duration-200 flex items-center"
              onClick={() => setIsOpen(false)}
            >
          ...
          <a
            href="#contact"
            className="bg-yellow-600 text-white font-medium py-3 px-8 rounded-full mt-4 text-center w-full flex items-center justify-center hover:bg-yellow-700 transition-all duration-300"
            onClick={() => setIsOpen(false)}
          >
            <Phone size={18} className="mr-2" />
            Get in touch
