...
//               <Link to="/#home" className="flex items-center justify-center text-center">
...
//                 <span className="text-white font-medium text-xl ml-2 relative" style={{ top: '1px' }}>
//                   REAL ESTATE
//                 </span>
//               </Link>
//             </div>
...
//                   className="w-9 h-9 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-yellow-600 hover:text-white transition-colors duration-300"
...
//                     className="text-gray-400 hover:text-yellow-600 transition-colors duration-200 flex items-center"
//                   >
...
const Footer: React.FC = () => {
  ...
  // Extract ID from URL hash (e.g., /#about → about)
  const extractId = (to: string) => {
    const hashIndex = to.indexOf('#');
    return hashIndex !== -1 ? to.substring(hashIndex + 1).split('?')[0] : '';
  };
  ...
  const handleSmoothScroll = (
    ...
    if (targetElement) {
      ...
      // If element not found (on different route), navigate first
      const path = to.split('#')[0];
      navigate(path + '#' + id);
      // Then scroll after slight delay
      setTimeout(() => {
      ...
      <div className="container mx-auto">
        <div className="py-16 px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <p className="text-gray-400 mb-6 leading-relaxed text-center md:text-left">
              Premium real estate experiences curated by Mrs. Deepali Parab. Connecting discerning clients with exceptional properties since 2012.
            </p>
            ...
            <ul className="space-y-3">
              {services.map((service, index) => (
                <li key={index}>
                  <a
                    href={service.to}
                    onClick={(e) => handleSmoothScroll(e, service.to)}
                    className="text-gray-400 hover:text-yellow-600 transition-colors duration-200 flex items-center"
                  >
                    <ChevronRight size={16} className="mr-2" />
                    {service.name}
